/*
  # Fix appointment slots for Dr. Himabindu Sridhar

  1. Changes
    - Fixes the ambiguous column reference error in previous migration
    - Creates appointment slots for doctor_id = 2 (Dr. Himabindu Sridhar)
    - Handles Sunday slots correctly (9:30 AM to 1:00 PM only)
    - Adds verification function to check slot creation
*/
...
BEGIN
  ...
  IF doctor2_slots_count = 0 THEN
    ...
    SELECT 
      2,  -- Set doctor_id to 2 (Dr. Himabindu Sridhar)
      date,
      time,
      false  -- Set all new slots as available
...
DECLARE
  ...
  slot_date DATE;
  slot_time TIME;
  p_doctor_id INTEGER := 2; -- Dr. Himabindu Sridhar
BEGIN
...
-- Create a function to verify slot creation if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    AND p.proname = 'verify_doctor_slots'
  ) THEN
