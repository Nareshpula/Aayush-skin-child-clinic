/*
  ...
  1. Changes
    ...
    - Includes the last slot in each time range (3:00 PM and 9:00 PM)
    - Adds debugging functions to verify timezone conversion
    - Ensures all slot generation logic respects IST timezone
*/
...
-- Grant execute permissions on the function
GRANT EXECUTE ON FUNCTION public.test_ist_timezone_conversion(DATE) TO service_role;
...
-- Grant execute permissions on the function
GRANT EXECUTE ON FUNCTION public.test_slot_generation_with_ist(INTEGER, DATE) TO service_role;

-- Create a function to debug available slots with IST timezone information
CREATE OR REPLACE FUNCTION public.debug_available_slots_ist(
  p_doctor_id INTEGER,
  p_date DATE
)
RETURNS TABLE (
...
BEGIN
  ...
  -- Set the end time for morning slots based on whether it's Sunday in IST
  IF is_sunday_ist THEN
    end_morning_time := '13:00:00'::TIME;
  ELSE
    end_morning_time := '15:00:00'::TIME;
  ...
  -- Evening slots (6:00 PM to 9:00 PM) - only for non-Sundays in IST
  IF NOT is_sunday_ist THEN
    slot_time := start_evening_time;
    -- Process evening slots - INCLUDE the end time (<=)
    WHILE slot_time <= end_evening_time LOOP
      -- Check if there's any exception for this doctor at this date and time
...
-- Grant execute permissions on the function
GRANT EXECUTE ON FUNCTION public.debug_available_slots_ist(INTEGER, DATE) TO service_role;