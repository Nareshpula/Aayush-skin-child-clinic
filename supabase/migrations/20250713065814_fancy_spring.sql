-- First, drop the existing validate_otp function
...
BEGIN
  ...
  SELECT (now() + INTERVAL '10 minutes') AT TIME ZONE 'Asia/Kolkata' INTO expiry_time;
...
BEGIN
  ...
  -- Get current time in IST with proper casting
  SELECT (now() AT TIME ZONE 'Asia/Kolkata')::timestamptz INTO current_time;
