/*
  ...
  1. Changes
    - Creates a dedicated function to generate sequential patient IDs
    - Uses MAX() to find the highest sequence number
    - Ensures IDs are always sequential and unique
    - Adds a function to fix existing out-of-order IDs
    - Includes debug functions for troubleshooting
*/

-- Create a function to get the next patient ID with proper sequence
...
CREATE OR REPLACE FUNCTION public.debug_patient_id_generation_v3()
RETURNS JSONB AS $$
DECLARE
  current_year TEXT;
...
GRANT EXECUTE ON FUNCTION public.debug_patient_id_generation_v3() TO anon, authenticated, service_role;
...
COMMENT ON FUNCTION public.debug_patient_id_generation_v3() IS 'Debugs the patient ID generation process and returns detailed information';
