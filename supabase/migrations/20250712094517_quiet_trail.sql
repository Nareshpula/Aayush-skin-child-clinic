/*
  ...
  1. New Features
    - Creates doctor_exceptions table to track when doctors are unavailable
    - Adds functions to add, remove, and list doctor exceptions
    - Updates slot availability function to check for doctor exceptions

  2. Changes
    - Ensures appointment slots consider doctor exceptions
...
GRANT EXECUTE ON FUNCTION public.get_available_slots_v3(INTEGER, DATE) TO anon, authenticated, service_role;
...
COMMENT ON FUNCTION public.get_available_slots_v3(INTEGER, DATE) IS 'Gets available time slots for a doctor on a specific date, considering exceptions and same-day restrictions';