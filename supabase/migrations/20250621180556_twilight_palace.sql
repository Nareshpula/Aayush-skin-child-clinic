/*
  ...
  1. Changes
    - Fixes Sunday detection by properly converting UTC to IST timezone
    - Ensures all slots are displayed including the last slot (3:00 PM and 9:00 PM)
    - Adds comprehensive debugging functions for timezone testing
...
CREATE OR REPLACE FUNCTION public.test_ist_timezone_conversion_v2(
  test_date DATE
)
RETURNS TABLE (
  input_date DATE,
...
-- Grant execute permissions on the function
GRANT EXECUTE ON FUNCTION public.test_ist_timezone_conversion_v2(DATE) TO service_role;

-- Create a function to test the slot generation with IST timezone
CREATE OR REPLACE FUNCTION public.test_slot_generation_with_ist_v2(
  p_doctor_id INTEGER,
  p_date DATE
)
RETURNS TABLE (
...
-- Grant execute permissions on the function
GRANT EXECUTE ON FUNCTION public.test_slot_generation_with_ist_v2(INTEGER, DATE) TO service_role;