/*
  # Simple Doctor Appointment System

  1. New Tables
    - `doctor_exceptions` - Stores time ranges when doctors are unavailable
  
  2. New Functions
    - `is_doctor_available` - Checks if a doctor is available at a specific time
    - `book_appointment` - Books an appointment with a doctor
    - `get_available_slots` - Gets available slots for a doctor on a specific date
    - `add_doctor_exception` - Adds a new doctor exception
    - `remove_doctor_exception` - Removes a doctor exception
*/
...
-- Function to get doctor exceptions
...
-- Function to add a doctor exception
...
-- Function to remove a doctor exception
