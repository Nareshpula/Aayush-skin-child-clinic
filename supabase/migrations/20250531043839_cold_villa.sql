/*
  # Fix Patient ID Generation and OTP Handling

  1. Changes
    - Fixes the patient_id generation function to properly handle the format
    - Adds proper error handling for the patient_id generation
    - Updates the get_appointments_with_details function to include patient_id
    - Ensures all functions work correctly with the updated schema
*/
...
BEGIN
  ...
  BEGIN
    SELECT COALESCE(MAX(
      CASE 
        WHEN patient_id ~ '^AAYUSH-[0-9]{2}-[0-9]{5}$' 
        THEN SUBSTRING(patient_id, 9)::INT 
        ELSE 0 
      END
